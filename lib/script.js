...
exports.runAction = async function runAction(script, player){
    ...
    if (await gameData.validate(player.data, 'player') && !_.isEqual(current.data, player.data)){
        await models.player.update(player.id, player);
        result.playerUpdated = true;
    }

    if (await gameData.validate(run.data, 'run') && !_.isEqual(currentRun.data, run.data)){
        ...
        result.actions = await async.map(sandbox.actions, async action => {
            ...
            switch (action.action){
                ...
                case 'video': {
                    ...
                    action.meetingName = meeting.meeting_id;
                    delete action.meetingId;
                    return action;
                }
