...
exports.updateTriggers = async function updateTriggers(userId) {
    ...
    if (!triggers) {
        ...
        await cache.invalidate('trigger', 'list', triggers);
    }
    const filteredTriggers = await async.filter(triggers, async trigger => {
...
exports.updateAllTriggers = async function updateAllTriggers(){
    ...
    await async.each(users, async user => {
        return await exports.updateTriggers(user.id);
    });
};
