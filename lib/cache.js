...
class Cache{
    ...
    store(name, id, data, timeout){
        ...
        if (self.type === 'local'){
            ...
            self.client.set(cacheId, JSON.stringify(data));
            self.client.expire(cacheId, timeout);
        }
    }
    ...
    async check(name, id){
        ...
        if (self.type === 'local'){
            ...
            try{
                const cacheId = getHash(name, id);
                const asyncGet = promisify(self.client.get).bind(self.client);
                const result = await asyncGet(cacheId);
                if (result){
