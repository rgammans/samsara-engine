...
exports.addNewVariable = async function addNewVariable(variable, oldVariable){
    ...
    return Promise.all(
        objects.map(async datum => {
            ...
            for (const visbility in ['public', 'private']){
                if (!_.has(datum.data, visbility)){
                    data.datum.data[visbility] = {};
                }
            }
            ...
            if (oldVariable){
                ...
                if (_.has(datum.data[oldVisibility], oldVariable.name)){
                    const oldVisibility = oldVariable.public?'public':'private';
                    if (_.isEqual(datum.data[oldVisibility][oldVariable.name], convert(oldVariable.base_value, oldVariable.type))){
