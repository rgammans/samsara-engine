...
class GameServer {
    ...
    async sendGameState(userId, ws, force){
        ...
        self.sendMeetingCounts(userId, ws);
    }
    ...
    async sendMeetingCounts(userId, ws){
        const self = this;
        if (self.messageTimers.meetings && self.messageTimers.meetings[userId]){
            return;
        }
        self.messageTimers.meetings[userId] = setTimeout(async () => {
            ...
            delete self.messageTimers.meetings[userId];
        }, 50);
    }
