...
class GameServer {
    ...
    async handleConnection(ws, request){
        ...
        if (user.type === 'player'){
            await self.sendGamestateCounts(user.player.run_id);
        }
    }
        ...
        try{
            switch(data.type){
                ...
                case 'statechange': {
                    ...
                    await self.sendGamestateCounts(data.payload.runId);
    ...
    async sendGamestateCounts(runId){
        const self = this;
        // Send updated gamestate counts
        const doc = JSON.stringify({
            action: 'gamedata',
            gamestate: await gameEngine.getGamestateCounts(runId)
        });

        await Promise.all(
            (await models.player.find({run_id: runId})).map(async player => {
