...
class GameServer {
    ...
    async handleMessage(ws, user, data){
        ...
        if (user.player){
            ...
            if (oldGamestate.current.id !== newGamestate.current.id){
                await self.sendGameState(user.id);
        ...
        try{
            switch(data.type){
                ...
                case 'statechange': {
                    ...
                    console.log('sent update to all run players');
                    ...
                    console.log('sent update to all staff');

                    let players = [];
                    if (!data.payload.run_id || !data.payload.newStateId){
                        await Promise.all(
                            _.keys(self.clients).map(async userId => {
                                return self.sendChatLocations(userId);
                            })
                        );
                        console.log(`sending to only current clients, because ${data.payload.run_id} ${data.payload.newStateId}`)
                        return;
                    }
                    ...
                    console.log(`sending updated chat locations to ${_.pluck(players, 'user_id').join(', ')}`);
                ...
                case 'gamedata':{
                    if (data.payload.userId){
                        self.sendGameData(data.payload.runId, data.payload.userId);
                    } else {
                        self.sendGameData(data.payload.runId);
                    }
                    break;
                }
            }
    ...
    async sendGameData(runId, userId){
        ...
        const players = userId?[await models.player.get(userId)]:await models.player.find({run_id:runId});
