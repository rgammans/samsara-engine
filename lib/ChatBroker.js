...
class ChatBroker extends EventEmitter{
    constructor(){
        super();
        if (ChatBroker._instance) {
            throw new Error('Singleton classes can\'t be instantiated more than once.');
        }
        ChatBroker._instance = this;
        const self = this;
        if (config.get('chat.brokerType') === 'redis'){
            this.type = 'redis';
            this.channel = config.get('chat.channel');
