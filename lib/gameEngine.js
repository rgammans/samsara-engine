...
async function checkRoom(roomId, userId){
    ...
    for (const stateRoom of gamestate.rooms){
    ...
    for (const transition in gamestate.current.transitions){
...
exports.changeState = async function changeState(userId, newStateId, delay){
    ...
    player.statetime = now.setSeconds(now.getSeconds() + delay);
...
exports.validate = async function validate(startStateId){
    const gamestates = _.indexBy((await Promise.all(
        (models.gamestate.list()).map( async gamestate => {
    ...
    const unusedGamestates = _.pluck(gamestates, 'id');
    const unusedRooms = _.pluck(rooms, 'id');
    const unusedImagemaps = _.pluck(imagemaps, 'id');
