...
function check(req, permission, bypass){
    ...
    if (permission === 'creator' && (user.is_admin || user.is_creator) && (bypass || !req.session.gm_mode)){
        return true;
    }

    if (permission === 'gm' && (user.is_admin || user.is_creator || user.is_gm)){
        return true;
    }
    if (permission === 'player' && (user.is_admin || user.is_creator || user.is_gm || user.is_player)){
        return true;
    }
    return false;
}
