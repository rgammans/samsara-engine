...
exports.getTransitionsFrom = async function getTransitionsFrom(gamestate){
    ...
    if (gamestate.image_id){
        for (const [idx, area] of gamestate.map.entries()){
...
async function checkLink(linkId, userId){
    ...
    for (const transition of gamestate.current.transitions){
        if (transition.link_id === linkId && (!transition.group_id || _.findWhere(gamestate.player.groups, {id: transition.group_id}))){
            return {
...
exports.openCode = async function openCode(code, userId){
    ...
    const checkedLink =  checkLink(link.id, userId);

    if (!checkedLink) { return null; }
    if (checkedLink.to_state_id){
        await exports.changeState(userId, checkedLink.to_state_id, checkedLink.delay);
    }
    return link;
};

exports.openArea = async function openArea(areaId, userId){
    const gamestate = await exports.getGameState(userId);
    ...
    if (state.image_id){
        actions = await getActions(state.map[areaId].actions, gamestate);
    }
    return actions;
};

async function getActions(actions, gamestate){
    const userId = gamestate.player.user_id;
    ...
    for (const action of actions){
        if (action.condition){
            if (!script.runCheck(action.condition, gamestate.player)){
                continue;
            }
        }
        switch(action.type){
            case 'link': {
                ...
                if (link.link.url === 'stub'){
                    filteredActions.push({action:'load', url:'/stub/' + link.link.id});
                } else {
                    filteredActions.push({action: 'load', url: link.link.url});
                }
            ...
            case 'text':{
                ...
                if (_.has(action, 'text_id')){
                    const text = await models.document.get(action.text_id);
            ...
            case 'script':{
                let result = null;
                try{
                    result = await script.game(action.script, gamestate.player);
                    await userData.update(userId, action.gamestate.player.data);
                    if (!stateChanged && result.gamestate_id){
