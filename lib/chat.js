...
async function fillMessage(message, cache){
    if (message.type === 'report'){
        ...
        switch (message.location){
            case 'group':
                message.location_name = (await getValue('group', message.location_id)).name;
                break;
            case 'gamestate':
                message.location_name = (await getValue('gamestate', message.location_id)).name;
                break;
            case 'direct': {
...
exports.getRecipients = async function getRecipients(message){
    ...
    switch(message.location){
        case 'gamestate': {
            const players = await models.player.find({gamestate_id: message.location_id, run_id:message.run_id});
...
exports.getHistory = async function getHistory(user, options){
    ...
    if (user.player){
        if (!options.type || options.type === 'group'){
            const groups = await Promise.all(user.player.groups.map(async group => {
...
exports.getLocations = async function getLocations(userId){
    ...
    if (user.type === 'none'){
        ...
        const gamestate = await models.gamestate.get(user.player.gamestate_id);
        if (gamestate.chat){
        ...
        const gamestates = (await models.gamestate.list())
            .filter(gamestate => {return !gamestate.template; })
            .filter(gamestate => {return !gamestate.start; })
            .filter(gamestate => {return !gamestate.finish; })
            .filter(gamestate => {return gamestate.chat; });

        doc.gamestate = await Promise.all(
            gamestates.map(async gamestate => {return {
                id: gamestate.id,
                name: gamestate.name,
