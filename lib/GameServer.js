...
class GameServer {
    ...
    async handleConnection(ws, request){
        ...
        await self.sendMeetingCounts(userId, ws);
    ...
    async handleMessage(ws, user, data){
        ...
        switch(data.action){
            ...
            case 'meeting': await self.handleMeeting(ws, user, data); break;
        }
    ...
    async handleMeeting(ws, user, data){
        const self = this;
        if (user.type !== 'player'){
            return;
        }
        try{
            ...
            const participant = await models.participant.findOne(params);


            switch (data.type){
                case 'join':{
                    if (!participant) {
                        await models.participant.create(params);
                        //update meetings for all
                    }
                    break;
                }
                case 'leave': {
                    if (participant){
                        await models.participant.delete(participant.id);
                        //update meetings for all
                    }
                }
            }
            ...
            await async.each(players, async player => {
                return self.sendMeetingCounts(player.user_id);
            });
        } catch(err){
            console.trace(err);
            const doc = {
    ...
    async sendMeetingCounts(userId, ws){
        const self = this;
    ...
    async updateMeetingCounts(){
        const self = this;
        for (const userId in self.clients){
            await self.sendMeetingCounts(userId);
        }
    }
