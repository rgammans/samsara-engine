...
class GameServer {
    ...
    init() {
        ...
        ChatBroker.on('message', async function(data){
            await self.handleBrokeredChat(data);
        });
    }
    ...
    async handleArea(ws, user, data){
        ...
        try{
            const areaUuid = data.areaId;
            const actions = await gameEngine.openArea(areaUuid, user.id);
            if (actions){
                for(const action of actions){
                    ws.send(JSON.stringify(action));
                }
    ...
    async handleChat(ws, user, data){
        ...
        try{
            ...
            if (message){
                ChatBroker.send('message', message);
            }
        } catch(err){
            console.trace(err);
            const doc = {
    ...
    async sendChat(userId, message, ws){
        ...
        if (ws){
            ...
            for (const clientId in self.clients[userId]){
                self.clients[userId][clientId].send(JSON.stringify(doc));
            }
        }
    }
    ...
    async sendLocationUpdate(runId, oldStateId, newStateId){
        const self = this;
        await ChatBroker.send('statechange', {
            runId: runId,
