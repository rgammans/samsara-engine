...
async function getGameStateRecord(id){
    let gamestate = cache.check('gamestateRecord', id);
    if (gamestate) { return gamestate; }

    gamestate = await models.gamestate.get(id);
    if (!gamestate){ return; }
    ...
    cache.store('gamestateRecord', id, gamestate);
    return gamestate;
}
...
exports.getTriggers = async function getTriggers(userId){
    let triggers = cache.check('trigger', 'list');
    if (!triggers) {
        triggers = (await models.trigger.list()).filter( trigger => { return trigger.player; } );
        cache.store('trigger', 'list', triggers);
    }

    return async.filter( triggers, async trigger => {
